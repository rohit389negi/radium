    ...
    try {
        ...
        if (isValidrequestBody(queryPara)) {
          ...
          if (isValid(category)) {
            ...
            let findBooks = await BookModel.find(filterDel).select({ _id: 1, title: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1, reviews: 1 });
...
const update = async function (req, res) {
 ...
 let newdata = await BookModel.findOneAndUpdate({ _id:bookUser }, { "title": req.body.title,"excerpt": req.body.excerpt,
 "ISBN":req.body.ISBN, "releasedAt": req.body.releasedAt }, { new: true })
...
//DELETE /books/:bookId
 const deletebookbyID= async function (req, res) {
    try {
        ...
        if(!deleteBook) {
            res.status(404).send({status: false, message: `The book has been already deleted`})
            return
        }
